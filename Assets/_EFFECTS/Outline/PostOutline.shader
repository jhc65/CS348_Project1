...
Shader "Custom/Post Outline"
{
    Properties
    {
		...
		_OutlineCol("Outline Color", Color)=(1,.5,1,1)
    }
    SubShader 
    {
        Pass 
        ...
        Pass 
        {
            ...
            half4 frag(v2f i) : COLOR 
            {
                ...
                for(int j=0;j<NumberOfIterations;j+=1)
                {
                    ...
                    ColorIntensityInRadius+= tex2D(
						_GrabTexture, 
                        float2(i.uvs.x,1-i.uvs.y)+float2(0,(j-NumberOfIterations/2)*TX_y)).r/NumberOfIterations;
                }
 
 
                //this is alpha blending, but we can't use HW blending unless we make a third pass, so this is probably cheaper.
                half4 outcolor=ColorIntensityInRadius*_OutlineCol*2+(1-ColorIntensityInRadius)*tex2D(_SceneTex,float2(i.uvs.x,i.uvs.y));
                return outcolor;
            }
