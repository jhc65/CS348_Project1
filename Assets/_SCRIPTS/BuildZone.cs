...
public class BuildZone : MonoBehaviour {
	...
	private void SnapPiece(Placeable p)
	{
		 ...
		 * 
		 * pieceSize (1/2) / fractionData.Value (3/2) = Percent to fill 1/3
		 */
		float PercentToFill = (float)(p.Value / fractionData.Value);
		...
		else
		{
            ...
            {
                case PivotType.Left:
                    targetPos.x += (float)(previous.Value / fractionData.Value);
                    break;
                case PivotType.Center:
                    targetPos.x += (float)(previous.Value / fractionData.Value) / 2;
                    targetPos.x += PercentToFill / 2;
                    break;
                case PivotType.Right:
                    targetPos.x += PercentToFill;
	...
	public void ClearBuildZone()
	{
		...
		/* Clear out fractionData.Value */
		fractionData.Value = FractionTools.Fraction.Zero;
